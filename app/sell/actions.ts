...
export async function UpdateUserSettings(prevState: any,formData: FormData) {
    ...
    if(!validateFields.success) {
        const state: State = {
            status: "error",
            errors: validateFields.error.flatten().fieldErrors,
            message: 'There is an error in your inputs'
        };
...
export async function BuyProduct(formData : FormData) {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        : "https://digital-marketplace-red.vercel.app/payment/success" ,
        cancel_url: process.env.NODE_ENV === "development" 
        ? 'http://localhost:3000/payment/cancel'
        : "https://digital-marketplace-red.vercel.app/payment/cancel" ,
    });
...
export async function CreateStripeAccountLink() {
    ...
    const accountLink = await stripe.accountLinks.create({
        ...
        : `https://digital-marketplace-red.vercel.app/billing`,
        return_url: process.env.NODE_ENV === "development" 
        ? `http://localhost:3000/return/${data?.connectedAccountId}`
        : `https://digital-marketplace-red.vercel.app/return/${data?.connectedAccountId}`,
